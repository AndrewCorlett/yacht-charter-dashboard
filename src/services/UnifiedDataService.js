...
import bookingService from './supabase/BookingService.js'
...
class DataTransformer {
  ...
  static charterToBooking(charter) {
    ...
    return {
      id: charter.id,
      booking_number: null, // Booking numbers are generated by BookingService with YYWWBCNN format
...
class UnifiedDataService {
  ...
  syncChartersFromBookings() {
    // Convert bookings to charters with deduplication to prevent React key conflicts
    const charterMap = new Map()
    
    this.bookings.forEach(booking => {
      const charter = DataTransformer.bookingToCharter(booking)
      // Use charter.id as key to ensure uniqueness
      charterMap.set(charter.id, charter)
    })
    
    // Convert Map back to array (automatically deduplicates by ID)
    this.charters = Array.from(charterMap.values())
  ...
  transformFieldNames(data) {
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      const validDatabaseFields = [
        ...
        'booking_type', 'booking_confirmed', 'deposit_paid', 'final_payment_paid', 'contract_sent', 
  ...
  async updateBooking(id, updates) {
    if (this.useSupabase && supabase) {
      try {
        console.log('[UnifiedDataService] Updating booking via BookingService:', id, updates)
        
        // Use BookingService to update the booking
        const updatedBooking = await bookingService.updateBooking(id, updates)
        
        console.log('[UnifiedDataService] BookingService returned updated booking:', updatedBooking)
        ...
        console.error('Failed to update booking via BookingService:', error)
        throw error
      }
    } else {
      // Use local mock data
  ...
  async addBooking(booking) {
    if (this.useSupabase && supabase) {
      try {
        console.log('[UnifiedDataService] Creating booking via BookingService:', booking)
        
        // Use BookingService to create the booking (this will generate the correct YYWWBCNN booking number)
        const newBooking = await bookingService.createBooking(booking)
        
        console.log('[UnifiedDataService] BookingService returned:', newBooking)
        ...
        console.error('Failed to create booking via BookingService:', error)
        throw error
      }
    } else {
      // Use local mock data
  ...
  async deleteBooking(id) {
    if (this.useSupabase && supabase) {
      try {
        console.log('[UnifiedDataService] Deleting booking via BookingService:', id)
        
        // Use BookingService to delete the booking
        await bookingService.deleteBooking(id)
        
        console.log('[UnifiedDataService] BookingService deleted booking successfully')
  ...
  async updateBookingNumber(id, newBookingNumber) {
    if (this.useSupabase && supabase) {
      try {
        console.log('[UnifiedDataService] Updating booking number via BookingService:', id, newBookingNumber)
        
        // Use BookingService to update the booking number
        const updatedBooking = await bookingService.updateBookingNumber(id, newBookingNumber)
        ...
        console.error('Failed to update booking number via BookingService:', error)
        throw error
      }
    } else {
      // Use local mock data
      ...
      this.bookings[index] = { ...this.bookings[index], booking_number: newBookingNumber }
   ...
   * @returns {Object} Created external booking
   */
  async createExternalBooking(externalBookingData) {
    if (this.useSupabase && supabase) {
      try {
        console.log('[UnifiedDataService] Creating external booking via BookingService:', externalBookingData)
        
        // Use BookingService to create the external booking
        const newBooking = await bookingService.createExternalBooking(externalBookingData)
        
        console.log('[UnifiedDataService] BookingService returned external booking:', newBooking)
        ...
        console.error('Failed to create external booking via BookingService:', error)
        throw error
      }
    } else {
      // Use local mock data
      ...
      this.bookings.push(externalBooking)
      
      // Sync to charters
      const newCharter = DataTransformer.bookingToCharter(externalBooking)
      this.charters.push(newCharter)
